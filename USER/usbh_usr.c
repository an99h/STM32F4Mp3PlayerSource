...
/* Private macros ------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
__IO uint8_t Command_index = 0;//切换程序工作状态，放音或是录音
...
extern __IO uint32_t Mp3DataLength ;
...
void USBH_USR_DeviceDisconnected (void)
{
	...
	/* If USB key Removed when playing a Mp3 */
	if( (Mp3DataLength!=0)&& (Command_index != 1))
	{
		Mp3Player_CallBack();
		Command_index = 0;
	}

	/* If USB key Removed when recording a Mp3 */
	if(Command_index == 1)
	{
		//Mp3RecorderStop();
...
int USBH_USR_MSC_Application(void)
{
	...
	case USH_USR_FS_INIT:
		...
		{
			LCD_ShowString(40,50,16,"f_mount error",1);
...
void COMMAND_AudioExecuteApplication(void)
{
	...
	case CMD_PLAY:
		//LCD_ShowString(40,80,16,"Mp3PlayerStart()",1);
		if (RepeatState == 0)
		{
			get_USB_free();
			Mp3PlayerStart("0:/MUSIC");
		}
